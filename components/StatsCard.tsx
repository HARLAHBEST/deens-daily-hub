...
const StatsCard: React.FC<StatsCardProps> = ({ 
  ...
  return (
    <div className="bg-white dark:bg-[#0f1f35] p-5 rounded-[28px] border border-slate-200 dark:border-white/5 shadow-sm transition-all hover:shadow-2xl hover:shadow-navy/5 hover:border-gold/30">
      <div className="flex justify-between items-start">
        <div className="space-y-1">
          <p className="text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-[2px]">
            {label}
          </p>
          <h3 className="text-2xl md:text-3xl font-black text-navy dark:text-white font-display tracking-tight leading-none mt-1.5">
            {value}
          </h3>
          {subValue && (
